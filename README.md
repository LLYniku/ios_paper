...
> [!NOTE]
> This repository now supports a second delivery mode for iOS: GitHub Actions generates JSON feed files and the `PaperDaily` app reads `latest.json`. App-specific setup lives in [README_APP.md](/Users/liuliaoyuan/dl_code/zotero-arxiv-daily-main/README_APP.md).
...
If you want an iPhone app instead of daily email output:

1. Read [README_APP.md](/Users/liuliaoyuan/dl_code/zotero-arxiv-daily-main/README_APP.md).
2. Use [config/paperdaily.example.yaml](/Users/liuliaoyuan/dl_code/zotero-arxiv-daily-main/config/paperdaily.example.yaml) as the `CUSTOM_CONFIG` value or as the `--config` file for local generation.
3. Run [scripts/preflight_paperdaily.sh](/Users/liuliaoyuan/dl_code/zotero-arxiv-daily-main/scripts/preflight_paperdaily.sh) before wiring GitHub Actions and Xcode signing.
...
By default, the PaperDaily feed workflow runs at 21:00 UTC everyday, which is 05:00 the next day in Asia/Shanghai. You can change this time by editing `.github/workflows/daily-app-feed.yml`.
